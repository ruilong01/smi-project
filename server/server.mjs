...
const EXTRACTION_SCRIPT_PATH = path.resolve(
  PROJECT_ROOT,
  "scripts/ingestion/runExtraction.mjs"
);
...
const EXTRACTION_STEPS = [
  ...
  { id: "ror", name: "Research Organization Registry" },
  { id: "mpa", name: "Maritime and Port Authority of Singapore" },
  { id: "ukri", name: "UKRI Gateway to Research" },
...
let extractionState = {
  ...
  completedAt: null,
  exitCode: null,
  message: "No extraction is running.",
  logs: [],
  lastDatasetGeneratedAt: null,
};
...
function appendExtractionLog(message) {
  const line = String(message).trim();
  if (!line) {
    return;
  }

  extractionState.logs = [...extractionState.logs, line].slice(-80);
}

function updateExtractionProgressFromLine(line) {
  appendExtractionLog(line);

  const startedSource = EXTRACTION_STEPS.find((step) =>
    line.includes(`Starting extraction: ${step.name}`)
  );
  if (startedSource) {
    extractionState.currentSource = startedSource.name;
    extractionState.message = `Extracting ${startedSource.name}...`;
    extractionState.progress = Math.max(
      extractionState.progress,
      Math.round(
        (EXTRACTION_STEPS.findIndex((step) => step.id === startedSource.id) /
  ...
  const completedSource = EXTRACTION_STEPS.find((step) =>
    line.includes(`${step.name}: Successfully extracted`)
  );
  if (completedSource) {
    extractionState.completedSources = [
      ...new Set([...extractionState.completedSources, completedSource.id]),
    ];
    extractionState.progress = Math.round(
      (extractionState.completedSources.length / EXTRACTION_STEPS.length) * 100
    );
    extractionState.message = `${completedSource.name} completed.`;
  }

  if (line.includes("Extraction Summary")) {
    extractionState.currentSource = "Finalising dataset";
    extractionState.message = "Finalising generated dataset...";
    extractionState.progress = Math.max(extractionState.progress, 95);
  }
}

function publicExtractionState() {
  return {
    ...extractionState,
    isRunning: extractionState.status === "running",
    totalSources: EXTRACTION_STEPS.length,
  };
}

function startExtractionRun() {
  if (extractionState.status === "running") {
    return { started: false, state: publicExtractionState() };
  }
  ...
  const child = spawn(process.execPath, [EXTRACTION_SCRIPT_PATH], {
    cwd: PROJECT_ROOT,
    env: process.env,
    windowsHide: true,
  });
  ...
  child.stdout.on("data", (chunk) => {
    chunk
      .toString()
      .split(/\r?\n/)
      .forEach((line) => updateExtractionProgressFromLine(line));
  });
  ...
  child.stderr.on("data", (chunk) => {
    chunk
      ...
      .forEach((line) => {
        appendExtractionLog(line);
        if (line.trim()) {
          extractionState.message = line.trim();
        }
      });
  });
  ...
  child.on("error", (error) => {
    ...
    extractionState.message = `Extraction failed to start: ${error.message}`;
    extractionState.progress = 100;
    appendExtractionLog(error.message);
  });

  child.on("close", (code) => {
    extractionState.status = code === 0 ? "completed" : "failed";
    extractionState.completedAt = new Date().toISOString();
    extractionState.exitCode = code;
    extractionState.progress = 100;

    if (code === 0) {
      try {
        const dataset = loadDataset();
        extractionState.lastDatasetGeneratedAt = dataset.meta?.generatedAt ?? null;
        extractionState.message = `Extraction completed: ${dataset.publicProjects?.length ?? 0} projects, ${dataset.countries?.length ?? 0} countries.`;
      } catch {
        extractionState.message = "Extraction completed, but dataset reload failed.";
      }
    } else {
      extractionState.message = `Extraction failed with exit code ${code}.`;
    }
  });

  return { started: true, state: publicExtractionState() };
}
...
function handleRequest(req, res) {
  ...
  // Manual extraction trigger used by the Source Status page.
  if (resource === "extraction") {
    if (req.method === "GET" && identifier === "status") {
      sendJson(res, 200, publicExtractionState());
      return;
    }

    if (req.method === "POST" && identifier === "run") {
      const result = startExtractionRun();
      sendJson(res, result.started ? 202 : 409, result.state);
      return;
    }

    notFound(res, "Unknown extraction endpoint");
    return;
  }
